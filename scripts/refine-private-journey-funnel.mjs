...
const locales = {
  en: {
    ...
    careEyebrow: "Included arrangements",
    careTitle: "What the starting estimate includes",
  ...
  zh: {
    ...
    careEyebrow: "行程內容", careTitle: "參考起價包含這些安排", careIntro: "每個價格都是地接安排參考起價，會依季節、房型與路線確認。",
  ...
  ja: {
    ...
    careEyebrow: "旅程に含まれる手配", careTitle: "参考料金に含まれるもの", careIntro: "料金は現地手配の目安です。季節、部屋、ルートによって最終確認します。", care: [["専用移動", "空港・駅の送迎とルート内の主な移動。"], ["宿の水準", "質のよい地域の宿またはアップグレード客室。"], ["言葉と予約", "重要な予約とルート詳細を中国語で確認。"], ["一日のペース", "一日一つの軸に、休息と移動の余白を確保。"]],
  ...
  ko: {
    ...
    careEyebrow: "포함 서비스", careTitle: "시작 견적에 포함되는 준비", careIntro: "현지 일정 참고가이며 계절, 객실과 루트에 따라 최종 확인합니다.", care: [["전용 이동", "공항 또는 역 픽업과 루트 내 주요 이동."], ["숙소 수준", "품질 좋은 현지 숙소 또는 업그레이드 객실."], ["언어와 예약", "중요 예약과 루트 세부 사항의 중국어 확인."], ["하루의 속도", "하루 한 가지 중심에 휴식과 이동 여유를 포함."]],
...
function routeLanguageMenu(slug, lang) {
  const labels = {
    en: ["English", "EN"], zh: ["繁體中文", "中"], ja: ["日本語", "日"],
    ko: ["한국어", "한"], th: ["ไทย", "ไทย"], ru: ["Русский", "RU"],
  };
  const links = Object.entries(labels).map(([code, [name, short]]) =>
    `<a href="${routeUrl(slug, code)}" lang="${code}"${code === lang ? ' aria-current="page"' : ""}><span>${name}</span><small>${short}</small></a>`,
  ).join("");
  return `<details class="language-menu"><summary aria-label="Choose language"><span>${labels[lang][1]}</span></summary><div class="language-options">${links}</div></details>`;
}
...
function renderMain(lang, locale) {
  return `<main>
    <section class="hero home-hero"><div class="hero-media" aria-hidden="true"><div class="hero-scene scene-yunnan"></div><div class="hero-scene scene-xinjiang"></div><div class="hero-scene scene-dunhuang"></div><div class="hero-scene scene-mongolia"></div><div class="hero-scene scene-sanya"></div><div class="hero-scene scene-northeast"></div></div><div class="wrap hero-inner"><p class="eyebrow">${esc(locale.heroEyebrow)}</p>${title(locale.heroTitle, "h1")}<p class="lead">${esc(locale.heroLead)}</p><div class="hero-actions"><a class="btn primary" href="#places">${esc(locale.heroPrimary)}</a><a class="btn" href="#plan-trip">${esc(locale.heroSecondary)}</a></div><div class="hero-proofline">${locale.heroProof.map((item) => `<span>${esc(item)}</span>`).join("")}</div></div></section>
...
function innerPage(lang, locale) {
  ...
  const menu = routeLanguageMenu("inner-mongolia", lang);
  ...
  <main><section class="hero destination-hero"><div class="wrap hero-inner"><p class="eyebrow">${esc(route.days[lang])} · ${esc(route.name[lang])}</p><h1>${esc(route.title[lang])}</h1><p class="lead">${esc(leads[lang])}</p><div class="hero-actions"><a class="btn primary" href="${cta}">${esc(locale.heroSecondary)}</a><a class="btn" href="${locale.homeUrl}#places">${esc(locale.heroPrimary)}</a></div><div class="facts"><div class="fact"><b>${esc(locale.fromLabel)}</b><span>${esc(route.price[lang])}</span></div><div class="fact"><b>${esc(locale.groupLabel)}</b><span>${esc(locale.groupText)}</span></div><div class="fact"><b>${esc(locale.routeLabel)}</b><span>${esc(route.route[lang])}</span></div></div></div></section>
    ...
    <section class="cta"><div class="wrap"><p class="eyebrow">${esc(locale.formEyebrow)}</p>${title(locale.formTitle)}<p>${esc(locale.formIntro)}</p><div class="hero-actions"><a class="btn primary" href="${cta}">${esc(locale.heroSecondary)}</a><a class="btn" href="${locale.homeUrl}#places">${esc(locale.heroPrimary)}</a></div></div></section></main><footer class="footer"><div class="wrap"><span>Bluehour China Journeys｜若青中國旅策</span><span><a href="/credits.html">Image credits</a> · <a href="/privacy.html">Privacy</a> · <a href="/llms.txt">AI-readable summary</a></span></div></footer><a class="sticky-review" href="${cta}">${esc(locale.navCta)}</a><script src="/assets/language-menu.js" defer></script></body></html>`;
}
...
await import("./add-xian-tibet-map.mjs");
...
await import("./refine-site-visual-rhythm.mjs");
await import("./expand-journey-stories.mjs");
await import("./bump-style-cache.mjs");
await import("./publish-single-starting-prices.mjs");
await import("./apply-retail-margin-prices.mjs?final-retail-policy");
await import("./personalize-destination-cta-copy.mjs");
